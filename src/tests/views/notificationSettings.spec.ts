...
describe('NotificationSettings', () => {
  ...
  describe('accountStore.updateNotificationPreference', () => {
    it('sends correct payload to update notification preference', async () => {
      ...
      axiosMock!.onPost('/api/account/update-notification-preference').reply(200, {
        record: { notify_on_reveal: true },
        old_value: false,
      });

      // Mock the account fetch endpoint with complete fixture
      axiosMock!.onGet('/api/account').reply(200, createAccountFixture({ notify_on_reveal: true }));
      ...
      expect(axiosMock!.history.post[0].url).toBe('/api/account/update-notification-preference');
    ...
    it('refetches account data after successful update', async () => {
      ...
      expect(axiosMock!.history.get[0].url).toBe('/api/account');
    });
    ...
    it('can toggle preference from true to false', async () => {
      ...
      axiosMock!
        .onGet('/api/account')
        .reply(200, createAccountFixture({ notify_on_reveal: false }));
    ...
    it('throws error on server failure', async () => {
      axiosMock!.onPost('/api/account/update-notification-preference').reply(500, {
    ...
    it('throws error on network failure', async () => {
      axiosMock!.onPost('/api/account/update-notification-preference').networkError();
    ...
    it('handles invalid field gracefully', async () => {
      ...
      axiosMock!.onPost('/api/account/update-notification-preference').reply(400, {
        message: 'Invalid field',
      });

      const store = useAccountStore();

      await expect(store.updateNotificationPreference('invalid_field', true)).rejects.toThrow();
    });

    it('handles unauthenticated user', async () => {
      // Server returns 401 for unauthenticated user
      axiosMock!.onPost('/api/account/update-notification-preference').reply(401, {
