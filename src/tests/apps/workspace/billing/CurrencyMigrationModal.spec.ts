  ...
  describe('Refund Failure (immediate mode)', () => {
    ...
    it('reaches checkout via explicit continue action after refund failure', async () => {
      ...
      const continueBtn = wrapper
        .findAll('button')
        .find((btn) => btn.text().includes('currency_migration.refund_failed_continue'));
    ...
    it('auto-redirects unchanged when refund_failed is false', async () => {
      ...
      expect(wrapper.text()).not.toContain('web.billing.currency_migration.refund_failed_warning');
    });
  });
