...
<script setup lang="ts">
  ...
  const svgOpacity = computed(() =>
    typeof props.opacity === 'number' ? Math.max(0, Math.min(1, props.opacity)) : 1
  );
  ...
  const ariaLabel = computed(() => props.ariaLabel || 'Keyhole logo');

  const hoverTitle = computed(() => props.title || 'Keyhole icon');
</script>
