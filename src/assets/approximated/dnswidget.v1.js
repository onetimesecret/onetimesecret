window.apxDns = {
  ...
  submitDomain: function (form) {
    ...
    let domain_submit_btn_el = document.querySelector(
      '#' + window.apxDns.config.widget_id + ' .apxdns-domain-submit'
    );

    let loader_el = document.querySelector(
      '#' + window.apxDns.config.widget_id + ' .apxdns-domain-loader'
    );
  ...
  setDomain: function (domain) {
    ...
    fetch(window.apxDns.config.api_url + '/get-provider-instructions', {
      ...
      body: JSON.stringify({
        records: window.apxDns.temp_records,
        token: window.apxDns.config.token,
      }),
    }).then(function (resp) {
      resp.json().then(function (data) {
  ...
  verifyRecords: function () {
    let verify_btn_el = document.querySelector(
      '#' + window.apxDns.config.widget_id + ' .apxdns-verify-btn'
    );
    if (verify_btn_el.classList.contains('apxdns-disable-btn')) {
      return;
    }
    let loader_el = document.querySelector(
      '#' + window.apxDns.config.widget_id + ' .apxdns-verify-loader'
    );
    loader_el.classList.remove('apxdns-hide');
    ...
    fetch(window.apxDns.config.api_url + '/token/check-records-match-exactly', {
      ...
      resp.json().then(function (data) {
        let partially_verified = data.records.some(function (item) {
          return item.match === true;
        });
        let completely_verified = data.records.every(function (item) {
          return item.match === true;
        });
        ...
        Object.keys(domain_records).forEach(function (apex_domain) {
          verify_section.insertAdjacentHTML(
            'beforeend',
            `
  ...
  deepClone: function (obj, hash = new WeakMap()) {
    ...
    if (obj instanceof Map) {
      ...
      obj.forEach((value, key) => {
        clonedMap.set(window.apxDns.deepClone(key, hash), window.apxDns.deepClone(value, hash));
      });
      return clonedMap;
    }
    // Set
    ...
    if (ArrayBuffer.isView(obj)) {
      const clonedView = new obj.constructor(obj.buffer.slice(0), obj.byteOffset, obj.length);
