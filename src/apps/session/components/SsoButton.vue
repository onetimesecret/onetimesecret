...
<template>
  <div>
    ...
    <button
      ...
      :disabled="isLoading"
      class="group relative flex w-full items-center justify-center gap-2 rounded-md border border-gray-300 bg-white px-4 py-2 text-lg font-medium text-gray-700 transition-colors duration-200 hover:bg-gray-50 focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600 dark:focus:ring-offset-gray-800"
      data-testid="sso-button">
      <span
        v-if="isLoading"
        class="flex items-center gap-2">
        <svg
          class="size-5 animate-spin text-gray-500 motion-reduce:animate-none dark:text-gray-400"
      ...
      <template v-else>
        ...
        {{
          displayName
            ? t('web.login.sign_in_with_provider', { provider: displayName })
