...
<script setup lang="ts">
  ...
  const confirmButtonLabel = computed(() =>
    isUpgrade.value
      ? t('web.billing.plans.confirm_upgrade')
      : t('web.billing.plans.confirm_downgrade')
  );
  ...
  const hasNewProrationFields = computed(() => {
    ...
    return (
      preview.value.immediate_amount !== undefined && preview.value.next_period_amount !== undefined
    );
  });
                  ...
                  <div class="space-y-3 text-sm">
                    <div class="flex justify-between">
                      <span class="text-gray-600 dark:text-gray-400">{{
                        t('web.billing.plans.current_plan_label')
                      }}</span>
                      <span class="font-medium text-gray-900 dark:text-white">
                        {{ currentPlan?.name }} ({{
                          formatCurrency(preview.current_plan.amount, preview.currency)
                        }}/{{ preview.current_plan.interval }})
                      </span>
                    </div>
                    <div class="flex justify-between">
                      <span class="text-gray-600 dark:text-gray-400">{{
                        t('web.billing.plans.new_plan_label')
                      }}</span>
                      <span class="font-medium text-gray-900 dark:text-white">
                        {{ targetPlan?.name }} ({{
                          formatCurrency(preview.new_plan.amount, preview.currency)
                    ...
                    <template v-if="showCreditBreakdown">
                      <!-- Section heading -->
                      <h4
                        class="pt-1 text-xs font-semibold tracking-wide text-gray-500 uppercase dark:text-gray-400">
                      ...
                      <dl class="space-y-2 border-l-2 border-gray-200 pl-3 dark:border-gray-600">
                        ...
                        <div class="flex justify-between">
                          ...
                          <dd class="font-medium text-gray-900 dark:text-white">
                            {{
                              formatCurrency(
                                preview.next_period_amount ?? preview.new_plan.amount,
                                preview.currency
                              )
                            }}
                          </dd>
                        ...
                        <div
                          v-if="remainingCreditAfterBill > 0"
                    ...
                    <template v-else-if="hasNewProrationFields">
                      ...
                      <div
                        v-if="preview.immediate_amount && preview.immediate_amount > 0"
                      ...
                      <div class="flex justify-between">
                        <span class="font-medium text-gray-900 dark:text-white">
                          {{
                            t('web.billing.plans.next_billing', {
                    ...
                    <div
                      ...
                      class="flex justify-between">
                      <span class="font-medium text-gray-900 dark:text-white">
                        {{ t('web.billing.plans.next_invoice')
              ...
              <div class="mt-6 sm:flex sm:flex-row-reverse sm:gap-3">
                <button
                  ...
                  :class="[
                    ...
                    (isChangingPlan || isLoadingPreview || error) &&
