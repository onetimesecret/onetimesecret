...
<script setup lang="ts">
  ...
  const { domain, details, initialize: initializeDomain } = useDomain(props.extid);
          ...
          <div class="text-base text-gray-600 dark:text-gray-300">
            <p>
              ...
              <span
                ...
                >{{ cluster?.proxy_host }}</span
              >{{ t('web.domains.if_you_already_have_a_cname_record_for_that_addr') }}
              <span
                :title="cluster?.proxy_name ?? ''"
                class="bg-white px-2 dark:bg-gray-800"
                >{{ cluster?.proxy_host }}</span
            ...
            <p
              ...
              <strong>{{ t('web.COMMON.important') }}:</strong>
              {{ t('web.domains.please_note_that_for_apex_domains') }}
              <span
                class="bg-white px-2 font-bold text-brand-600 dark:bg-gray-800 dark:text-brand-400"
                >{{ domain?.display_domain }}</span
              >{{ t('web.domains.a_cname_record_is_not_allowed_instead_youll_need') }}
            </p>
          </div>
      ...
      <div
        ...
        <div class="mt-6 flex items-center gap-4">
          <button
            ...
            class="inline-flex items-center rounded-md bg-brand-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-brand-700 focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 dark:bg-brand-500 dark:hover:bg-brand-600"
            @click="triggerVerification">
            <svg
              v-if="verificationInProgress"
              class="mr-2 -ml-1 h-4 w-4 animate-spin motion-reduce:animate-none"
              aria-hidden="true"
