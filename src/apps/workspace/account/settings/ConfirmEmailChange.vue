...
<script setup lang="ts">
  ...
  const { confirmEmailChange, isLoading, error } = useAuth();
    ...
    <div class="w-full max-w-md text-center">
      ...
      <div v-if="isLoading">
        <OIcon
          ...
          class="mx-auto size-8 animate-spin text-brand-500 motion-reduce:animate-none"
          aria-hidden="true" />
      ...
      <div v-else-if="confirmed">
        ...
        <router-link
          to="/signin"
          class="mt-4 inline-flex items-center gap-2 text-sm font-medium text-brand-600 hover:text-brand-700 dark:text-brand-400 dark:hover:text-brand-300"
      ...
      <div v-else-if="error">
        <div
          ...
          <OIcon
            ...
            class="size-10 text-red-600 dark:text-red-400"
            aria-hidden="true" />
        </div>
        <h1 class="mt-6 text-2xl font-bold text-gray-900 dark:text-white">
          {{ t('web.settings.profile.email_confirm_error_title') }}
        </h1>
        <p class="mt-3 text-red-600 dark:text-red-400">
          {{ t('web.settings.profile.email_confirm_invalid') }}
        </p>
        <router-link
          to="/account/settings/profile/email"
          class="mt-6 inline-flex items-center gap-2 rounded-md bg-brand-600 px-4 py-2 text-sm font-medium text-white hover:bg-brand-700 dark:bg-brand-500 dark:hover:bg-brand-600">
