...
<script setup lang="ts">
  ...
  const showSplitInput = computed(() => !props.flexibleFromDomain && !!props.displayDomain);
  ...
  const isFormValid = computed(() => {
    ...
    if (
      localForm.value.reply_to.trim() &&
      !emailSchema.safeParse(localForm.value.reply_to.trim()).success
    ) {
      return false;
    }
  ...
  // save is possible (valid + dirty + not busy) — same gating the in-form submit
  // used — and let the page relay it to DomainHeader's `save-disabled`.
  const canSave = computed(
    () => isFormValid.value && props.hasUnsavedChanges && !props.isSaving && !props.isDeleting
  );
...
<template>
  <form
      ...
      <div class="mt-6">
        ...
        <div
          ...
          <input
            ...
            class="block w-full min-w-0 flex-1 rounded-l-md border-gray-300 shadow-sm focus:border-brand-500 focus:ring-brand-500 sm:text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder:text-gray-400"
        ...
        <div
          ...
          <div class="flex">
            ...
            <div class="ml-3">
              ...
              <p
                v-if="testResult.details?.sent_to"
    ...
    <!-- Secondary actions: Delete + Discard. The primary Save ("Update") lives
         in the page header (opt-in DomainHeader affordance); this row only
         appears when there's a saved config to delete or an edit to discard. -->
    <div
      v-if="isConfigured || hasUnsavedChanges || showDeleteConfirm"
