  ...
  <div class="mt-8 space-y-6">
      ...
      <div class="flex">
        ...
        <div class="ml-3">
          ...
          <p class="mt-2 text-sm text-yellow-700 dark:text-yellow-300">
            {{ $t('auth.webauthn.requiresModernBrowser') }}
          </p>
        </div>
      </div>
    </div>
    ...
    <div v-if="supported">
      ...
      <button
        ...
        <span v-if="isLoading">{{ $t('auth.webauthn.processing') }}</span>
        <span v-else>{{ $t('auth.webauthn.signIn') }}</span>
      </button>

      <!-- Help text -->
      <div class="mt-4 text-center">
        <p class="text-xs text-gray-500 dark:text-gray-400">
          {{ $t('auth.webauthn.supportedMethods') }}
        </p>
      </div>
    </div>
  </div>
