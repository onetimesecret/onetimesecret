...
<template>
  ...
  <a :href="secretMetadata.uri"
      :class="[linkClass, 'hover:text-brand-500 dark:hover:text-brand-500 transition-colors']"
      :title="secretMetadata.is_destroyed ? 'RECEIVED' : 'NOT RECEIVED'">
    {{ displayKey }}
    <span v-if="secretMetadata.show_recipients"
  ...
  <a v-if="!secretMetadata.is_destroyed"
      :href="`${secretMetadata.uri}/burn`"
