<template>
  <form @submit.prevent="submitForm"
    ...
    <div class="space-y-4">
      <label for="logo"
             class="block text-lg font-brand font-semibold text-gray-700 dark:text-gray-200">Logo</label>
      <div class="flex items-center space-x-4">
        <div
             class="relative w-24 h-24 bg-gray-100 dark:bg-gray-700 rounded-lg overflow-hidden flex items-center justify-center border-2 border-dashed border-gray-300 dark:border-gray-600">
          <img v-if="logoDataUrl"
               :src="logoDataUrl"
               :alt="localBrandSettings.image_filename"
        ...
        <div class="flex-grow">
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400 truncate">
            {{ localBrandSettings.image_filename || 'No file chosen' }}</p>
        </div>
      </div>
      <button v-if="logoDataUrl"
              @click="removeLogo"
              type="button"
              class="text-sm text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 transition duration-300 ease-in-out focus:outline-none focus:underline"
    ...
    <div class="space-y-2">
      <label for="primary_color"
             class="block text-lg font-brand font-semibold text-gray-700 dark:text-gray-200">
        Primary Color
      </label>
      <div class="flex items-center space-x-4">
        <input type="color"
