...
const primaryNavItems = computed((): NavItem[] => {
  const items: NavItem[] = [
    {
      id: 'dashboard',
      path: '/dashboard',
      label: t('web.COMMON.title_dashboard'),
      icon: 'squares-2x2',
    },
    {
      id: 'secrets',
      path: '/recent',
      label: t('web.COMMON.title_recent'),
  ...
  items.push({
    id: 'settings',
    path: '/account',
    label: t('web.COMMON.title_account'),
    icon: 'cog-6-tooth',
  });
...
const quickActions = computed(() => [
  {
    id: 'create',
    label: t('web.COMMON.create_secret'),
    path: '/',
    variant: 'primary',
    icon: 'plus',
  }
  ...
  <nav
    ...
    <div class="flex items-center -mb-px">
      <router-link
        ...
        :key="item.id"
        :to="item.path"
        class="group relative flex items-center gap-2 px-4 py-3 text-sm font-medium
        ...
        <span
          ...
          class="ml-2 inline-flex items-center justify-center min-w-[20px] h-5 px-1.5
