...
<script setup lang="ts">
  ...
  const spriteComponents = {
    CarbonSprites: defineAsyncComponent(
      () => import('@/shared/components/icons/sprites/CarbonSprites.vue')
    ),
    CriticalSprites: defineAsyncComponent(
      () => import('@/shared/components/icons/sprites/CriticalSprites.vue')
    ),
    FontAwesome6Sprites: defineAsyncComponent(
      () => import('@/shared/components/icons/sprites/FontAwesome6Sprites.vue')
    ),
    HeroiconsSprites: defineAsyncComponent(
      () => import('@/shared/components/icons/sprites/HeroiconsSprites.vue')
    ),
    MaterialSymbolsSprites: defineAsyncComponent(
      () => import('@/shared/components/icons/sprites/MaterialSymbolsSprites.vue')
    ),
    MdiSprites: defineAsyncComponent(
      () => import('@/shared/components/icons/sprites/MdiSprites.vue')
    ),
    PhosphorSprites: defineAsyncComponent(
      () => import('@/shared/components/icons/sprites/PhosphorSprites.vue')
    ),
    TablerSprites: defineAsyncComponent(
      () => import('@/shared/components/icons/sprites/TablerSprites.vue')
    ),
    AlternateLogoSprites: defineAsyncComponent(
      () => import('@/shared/components/icons/sprites/AlternateLogoSprites.vue')
    ),
  };
...
<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    ...
    <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <!-- Header -->
      <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Icon Gallery</h1>
        <p class="mt-2 text-gray-600 dark:text-gray-400">
          <template v-if="isLoading"> Loading icons from sprite files... </template>
          <template v-else>
            {{ totalIconCount }} icons discovered across {{ libraryNames.length }} libraries. Click
            any icon to copy its ID.
          </template>
        </p>
      </div>
      ...
      <div
        ...
        <span class="ml-3 text-gray-600 dark:text-gray-400"> Loading sprite files... </span>
      </div>
      ...
      <template v-else>
        ...
        <div class="mb-8 flex flex-col gap-4 sm:flex-row">
          <div class="flex-1">
            <input
              ...
              class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 placeholder-gray-500 focus:border-brand-500 focus:ring-2 focus:ring-brand-500 focus:outline-none dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:placeholder-gray-400" />
          </div>
          <div class="flex gap-2">
            ...
            <select
              ...
              class="rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 focus:border-brand-500 focus:ring-2 focus:ring-brand-500 focus:outline-none dark:border-gray-600 dark:bg-gray-800 dark:text-white">
        ...
        <div class="space-y-12">
          <section
            ...
            :key="library">
            <div
              class="mb-4 flex items-baseline justify-between border-b border-gray-200 pb-2 dark:border-gray-700">
        ...
        <div
          ...
          <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-white">No icons found</h3>
          <p class="mt-2 text-gray-500 dark:text-gray-400">Try adjusting your search or filter.</p>
        </div>

        <!-- Usage instructions -->
        <div
          class="mt-16 rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
          <h2 class="mb-4 text-lg font-semibold text-gray-900 dark:text-white">Usage</h2>
          <pre
            class="overflow-x-auto rounded bg-gray-100 p-4 text-sm dark:bg-gray-900"><code class="text-gray-800 dark:text-gray-200">&lt;OIcon
...
/&gt;</code></pre>
          <p class="mt-4 text-sm text-gray-600 dark:text-gray-400">
            The icon ID format is
            <code class="rounded bg-gray-100 px-1 dark:bg-gray-900">{collection}-{name}</code>. For
            example,
            <code class="rounded bg-gray-100 px-1 dark:bg-gray-900"
              >heroicons-check-circle-solid</code
            >
            uses collection
            <code class="rounded bg-gray-100 px-1 dark:bg-gray-900">heroicons</code> and name
            <code class="rounded bg-gray-100 px-1 dark:bg-gray-900">check-circle-solid</code>.
          </p>
          <p class="mt-4 text-sm text-gray-500 dark:text-gray-500">
            Icons are discovered dynamically from rendered sprite files. New icons added to sprite
