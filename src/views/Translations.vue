...
<template>
  ...
  <div class="container mx-auto px-4 pt-10 max-w-2xl">
    <article class="prose dark:prose-invert lg:prose-lg xl:prose-xl">
      ...
      <!--{{ #translations }}-->
        <h4 class="text-xl font-semibold mb-2">
          {{ name }} (<a href="/?locale={{code}}"
             class="text-brand-500 dark:text-brand-400 hover:underline">switch</a>)
        </h4>
        <ul class="list-disc pl-5 mb-4">
          <!--{{ #translators }}-->
            <li>
              <a href="{{url}}"
                 class="text-brand-500 dark:text-brand-400 hover:underline">{{ name }}</a>
              ({{ date }})
            </li>
          <!--{{/translators}}-->
        </ul>
      ...
      <h4 class="text-xl font-semibold mb-4">Currently Supported Languages</h4>

      <ul class="list-disc pl-5 mb-4">
        <!--{{ #supported_locales }}-->
          <li>
            <a href="/?locale={{to_s}}"
               class="text-brand-500 dark:text-brand-400 hover:underline">{{ to_s }}</a>
          </li>
        <!--{{/supported_locales}}-->
      </ul>
