...
<template>
  <div class="">
    ...
    <div v-else-if="record && details"
         class="space-y-8 bg-gradient-to-b from-slate-50 to-slate-100
         dark:from-slate-900 dark:to-slate-950 rounded-lg p-6">
      <!-- Secret Link Header -->
      <section class="animate-fade-in relative"
        ...
        <div v-if="details?.has_passphrase"
             class="absolute -top-4 right-2 flex items-center gap-2 text-sm text-amber-600
             dark:text-amber-400">
          <OIcon collection=""
                 name="mdi-lock"
                 class="w-4 h-4" />
      ...
      <!-- Secret Value -->
      <section v-if="details.show_secret"
        ...
        <textarea readonly
                  ...
                  class="font-mono text-base leading-tight tracking-wide whitespace-pre
                  shadow-sm transition-colors px-4 py-3 w-full resize-none rounded-lg
        ...
        <div class="flex items-center justify-between w-full mt-2">
          <p class="text-sm text-amber-600 dark:text-amber-400 opacity-80 flex items-center gap-2"
             role="alert"
             aria-live="polite">
      ...
      <section v-if="!details.show_secret && !record.is_destroyed"
               class="bg-slate-200 bg-opacity-50 dark:bg-slate-800 dark:bg-opacity-50 rounded-lg p-4">
        <div class="flex items-center justify-between font-mono text-slate-400 dark:text-slate-500">
          <span>•••••••••••••••••••</span>
      ...
      <!-- Status & Timeline -->
      <section class="bg-slate-50 dark:bg-slate-800 rounded-lg p-4"
