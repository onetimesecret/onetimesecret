...
<template>
  <div>
    ...
    <div
      v-else-if="!record || details?.is_destroyed"
      role="alert"
    ...
    <div v-else>
      <div class="mb-6">
        <span class="text-lg text-gray-600 dark:text-gray-400">{{ $t('web.COMMON.secret') }}: {{ record?.secret_shortkey }}</span>
        <h2
      ...
      <form
        ...
        <a
          :href="`/${record?.metadata_path}`"
          class="mx-auto block w-3/4 rounded bg-gray-200 px-4 py-2 text-center text-gray-700 transition duration-200 hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600"
