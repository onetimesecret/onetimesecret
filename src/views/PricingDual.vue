<template>
  <div class="relative isolate bg-white px-6 py-18 sm:py-12 lg:px-8">
    <div class="pb-12 flex justify-center text-sm ">
      <ButtonGroup first-val="100% Human Content" mid-val1="50/50" last-val="100% AI Generated" />
    </div>
    <div class="absolute inset-x-0 -top-3 -z-10 transform-gpu overflow-hidden px-36 blur-3xl"
         aria-hidden="true">
      <!--<div class="mx-auto aspect-[1155/678] w-[72.1875rem] bg-gradient-to-tr from-[#655b5f] to-[#c25151] opacity-30"
           style="clip-path: polygon(74.1% 44.1%, 100% 61.6%, 97.5% 26.9%, 85.5% 0.1%, 80.7% 2%, 72.5% 32.5%, 60.2% 62.4%, 52.4% 68.1%, 47.5% 58.3%, 45.2% 34.5%, 27.5% 76.7%, 0.1% 64.9%, 17.9% 100%, 27.6% 76.8%, 76.1% 97.7%, 74.1% 44.1%)" />-->
      <MovingGlobules from-colour="#23b5dd" to-colour="#dc4a22" speed="4s" />
    </div>
    <div class="mx-auto max-w-2xl text-center lg:max-w-4xl">
      <h2 class="text-2xl font-semibold leading-7 text-brand-600">Pricing</h2>
      <p class="mt-2 text-4xl font-bold tracking-tight text-gray-900 sm:text-5xl">Identify yourself, wherever you are</p>
    </div>
      ...
      <fieldset aria-label="Payment frequency">
        <RadioGroup v-model="frequency" class="grid grid-cols-2 gap-x-1 rounded-full bg-white/5 p-1 text-center text-xs font-semibold leading-5 text-white">
          <RadioGroupOption as="template" v-for="option in frequencies" :key="option.value" :value="option" v-slot="{ checked }">
            <div :class="[checked ? 'bg-brand-600' : 'bg-white text-gray-900 opacity-55', 'cursor-pointer rounded-full px-2.5 py-1']">{{ option.label }}</div>
          </RadioGroupOption>
        </RadioGroup>
      </fieldset>
    </div>
    ...
    <div
      ...
      <div v-for="(tier, tierIdx) in tiers"
        ...
        <p class="mt-4 flex items-baseline gap-x-2">
          ...
          <span :class="[tier.featured ? 'text-gray-400' : 'text-gray-500', 'text-base']">{{ frequency.priceSuffix }}</span>
        </p>
...
const tiers = [
  ...
  },
  {
    ...
    id: 'tier-integrity',
    href: '#',
    price: { monthly: '$145', annually: '$1595' },
