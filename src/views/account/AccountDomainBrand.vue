    ...
    <div class="max-w-5xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden">
        ...
        <div
          ...
          <!-- Preview Controls - Fixed to ensure it's always clickable -->
  ...
  <div v-if="!isPreviewMode"
    ...
    <button v-if="logoSrc"
      ...
      Remove
        ...
        <!-- Replace the Settings Section part of the template -->
          ...
          <div class="p-6 space-y-8">
            ...
            <Transition name="fade"
              ...
              <div v-if="!loading"
                ...
                <BrandSettingsBasic v-if="activeTab === 'basic'"
                                    :brandSettings="brandSettings"
                                    :isSubmitting="isSubmitting"
                                    @update:brandSettings="updateBrandSettings" />
                <BrandSettingsAppearance v-if="activeTab === 'appearance'"
                                         :brandSettings="brandSettings"
                                         :isSubmitting="isSubmitting"
                                         @update:brandSettings="updateBrandSettings" />
                <BrandSettingsAdvanced v-if="activeTab === 'advanced'"
                                       :brandSettings="brandSettings"
                                       :isSubmitting="isSubmitting"
                                       @update:brandSettings="updateBrandSettings" />
              </div>
        ...
        <div v-show="isPreviewMode"
          ...
          <div class="text-center text-gray-500 dark:text-gray-400">
            <p class="text-sm font-medium">Preview Mode Active</p>
            <p class="text-xs mt-1">This is how your brand will appear to customers</p>
          </div>
        </div>
...
// Add a watch to handle preview mode changes
watch(isPreviewMode, (newValue) => {
  if (newValue) {
    // Store current state before preview
    previousTab.value = activeTab.value;
  } else {
    // Restore previous state after preview
...
watch(isPreviewMode, (newValue: boolean) => {
  if (newValue) {
    // Store current tab before preview
    previousTab.value = activeTab.value;
    // Optionally switch to a specific tab for preview
    activeTab.value = 'basic';
  } else {
    // Restore previous tab after preview
    activeTab.value = previousTab.value;
  }
});
...
/* Add a class for preview mode cursor */
.pointer-events-none {
  cursor: not-allowed;
}
</style>
