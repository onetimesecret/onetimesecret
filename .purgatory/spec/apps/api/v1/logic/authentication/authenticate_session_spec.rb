# .purgatory/spec/apps/api/v1/logic/authentication/authenticate_session_spec.rb
#
...
RSpec.xdescribe V1::Logic::Authentication::AuthenticateSession do
  ...
  before do
    ...
    allow(V1::Customer).to receive(:anonymous).and_return(anonymous_customer)
  ...
  describe '#process' do
    ...
    context 'when authentication is successful' do
      context 'and customer is pending' do
        ...
        it 'logs pending customer login' do
          expect(OT).to receive(:info).with("[login-pending-customer] abc123 test@example.com customer (pending)")
      ...
      context 'and customer is not pending' do
        ...
        it 'logs successful login' do
          expect(OT).to receive(:info).with("[login-success] abc123 te***@example.com customer (replacing sessid)")
          expect(OT).to receive(:info).with("[login-success] abc123 te***@example.com customer (new sessid)")
          subject.process
        end
    ...
    context 'when authentication fails' do
      ...
      it 'logs failure and raises form error' do
        expect(OT).to receive(:ld).with("[login-failure] abc123 te***@example.com customer (failed)")
        expect(subject).to receive(:raise_form_error).with("Try again")
        subject.process
      end
    end
