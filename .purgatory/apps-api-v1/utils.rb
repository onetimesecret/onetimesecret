# .purgatory/apps-api-v1/utils.rb
#
...
module V1
  ...
  #
  # Note: there are limitations to these implementations that existed in the
  # Onetime::Utils module for many years. We have made improvements and
  # alternative approaches to address these limitations in Onetime::Utils.
  # and related code that are not featured here. For example we are using
  # refinements for indifferent hash access rather than mucking directly
  # with the Hash instances.
  #
  module Utils
    extend self
    ...
    extend self
      ...
      def natural_time(time_in_s)
        ...
        if val < 10
          ...
          result = Time.at(time_in_s.to_i).strftime("#{weeks} #{weeks == 1 ? 'week' : 'weeks'} ago").downcase
        end
        result
      end
