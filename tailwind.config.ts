...
export default {
  ...
  theme: {
    ...
    extend: {
      // Brand palettes (brand, branddim, brandcomp, brandcompdim) live in
      // src/assets/style.css under `@theme static`. The `static` modifier
      // emits utilities at build time so `bg-brand-500` etc. resolve, while
      // the runtime `useBrandTheme` composable overrides the CSS variables
      // on `<html>` to swap the active palette per custom-domain branding.
