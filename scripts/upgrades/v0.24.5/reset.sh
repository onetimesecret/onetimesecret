#!/usr/bin/env bash
# scripts/upgrades/v0.24.5/reset.sh
#
# Resets the v0.24.5 upgrade state so upgrade.sh can be re-run from scratch.
#
...
#
# Usage:
#   scripts/upgrades/v0.24.5/reset.sh [OPTIONS]
#
# Options:
...
#   --data-dir=DIR      Data directory (default: data/upgrades/v0.24.5)
...
#
# Environment:
#   TARGET_VALKEY_URL   Valkey/Redis URL for the v0.24.5 target (DB 0)
#
# Safety:
...
#   scripts/upgrades/v0.24.5/reset.sh --keep-dumps
#
#   # Full reset: local files + flush target Valkey DB 0
#   TARGET_VALKEY_URL=redis://new-host:6380 \
#     scripts/upgrades/v0.24.5/reset.sh --flush-target
#
#   # Remove only the _original_* audit keys from target
#   TARGET_VALKEY_URL=redis://new-host:6380 \
#     scripts/upgrades/v0.24.5/reset.sh --flush-originals
#
#   # Unattended full reset (CI/staging)
#   scripts/upgrades/v0.24.5/reset.sh --flush-target --yes
#
# See also:
#   scripts/upgrades/v0.24.5/info.sh    -- inspect source/target keyspace (read-only)
...
DATA_DIR="${DATA_DIR:-data/upgrades/v0.24.5}"
...
echo ""
echo "+-----------------------------------------------------------------------+"
echo "|  v0.24.5 Reset -- Clean upgrade artifacts for re-run                  |"
echo "+-----------------------------------------------------------------------+"
echo "|"
echo "|  Mode:            $MODE"
...
echo "|    - Remove intermediate JSON files (lookups, manifests, diagnostics)"
if ! $KEEP_LOGS; then
...
if [ ! -d "$DATA_DIR" ]; then
  ...
  # Remove cross-phase lookup, diagnostic, and manifest JSON files (always)
  # These are generated by create_indexes.rb, generate.rb, and transform.rb
  # as intermediate artifacts: email_to_objid.json, customer_objid_to_org_objid.json,
  # email_to_org_objid.json, fqdn_to_objid.json, followup_*.json, dump_manifest_*.json
  JSON_COUNT=$(find "$DATA_DIR" -name '*.json' 2>/dev/null | wc -l | tr -d ' ')
  if [ "$JSON_COUNT" -gt 0 ]; then
    run_or_preview "Removing $JSON_COUNT intermediate JSON file(s)" \
      find "$DATA_DIR" -name '*.json' -delete 2>/dev/null || true
    TOTAL_REMOVE=$((TOTAL_REMOVE + JSON_COUNT))
  fi
