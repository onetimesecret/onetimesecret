#!/usr/bin/env bash
# scripts/upgrades/v0.24.5/upgrade.sh
#
# Orchestrates the full v0.24.5 data migration (Familia v1 -> v2).
#
...
#
# Usage:
#   scripts/upgrades/v0.24.5/upgrade.sh [OPTIONS]
#
# Options:
...
#   TARGET_VALKEY_URL   Valkey/Redis URL for the v0.24.5 target (writes DB 0)
#
...
#   SOURCE_REDIS_URL=redis://old-host:6379 \
#   TARGET_VALKEY_URL=redis://new-host:6380 \
#     scripts/upgrades/v0.24.5/upgrade.sh --execute
#
#   # Resume from phase 3 after inspecting phase 2 output
#   scripts/upgrades/v0.24.5/upgrade.sh --execute --start-phase=3
#
#   # Unattended run (CI/staging)
#   scripts/upgrades/v0.24.5/upgrade.sh --execute --skip-gates
#
# See also:
#   scripts/upgrades/v0.24.5/reset.sh   -- clean artifacts for re-run
...
if [ -z "${TARGET_VALKEY_URL:-}" ]; then
  ...
  echo "  This is the v0.24.5 Valkey/Redis instance to write to (DB 0)"
  exit 1
fi
...
echo ""
echo "+-----------------------------------------------------------------------+"
echo "|  v0.24.5 Upgrade -- Familia v1 -> v2 Data Migration                   |"
echo "+-----------------------------------------------------------------------+"
echo "|"
echo "|  Mode:       $MODE"
...
if [ "$START_PHASE" -le 1 ]; then
  ...
  ruby scripts/upgrades/v0.24.5/dump_keys.rb \
    --all \
...
if [ "$START_PHASE" -le 2 ]; then
  ...
  if $EXECUTE; then
    ...
    VALKEY_URL="$SOURCE_REDIS_URL" \
      bash scripts/upgrades/v0.24.5/run_pipeline.sh
  else
...
if $EXECUTE; then
  ...
  echo "|        scripts/upgrades/v0.24.5/info.sh --target"
  echo "|"
