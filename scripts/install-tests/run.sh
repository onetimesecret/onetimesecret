...
#
# Lanes:
#   baremetal   ruby:3.4.10-slim — the documented floor. bin/install must
#               succeed, produce a .env with a real SECRET, and be idempotent.
#   ruby-old    ruby:3.3-slim   — bin/install must FAIL with a clear
...
read -r -d '' ASSERT_AND_IDEMPOTENT <<'ASSERT' || true
  ...
  bin/install
  echo "OK: second bin/install succeeded (idempotent)"
ASSERT
...
lane_baremetal() {
  run_in_image "$IMAGE_BAREMETAL" "baremetal" "
    $SETUP_TOOLCHAIN
    echo '--- bin/install (documented floor) ---'
    bin/install
...
lane_posix() {
  ...
  run_in_image "$IMAGE_BAREMETAL" "posix-locale" "
    ...
    $SETUP_TOOLCHAIN
    echo '--- bin/install under POSIX locale ---'
    bin/install
    $ASSERT_AND_IDEMPOTENT
  "
}
...
lane_ruby_old() {
  # Asserted-error lane: bin/install MUST fail fast with the exact-match
  ...
  echo "$out"
  if [[ "$status" -eq 0 ]]; then
    echo "FAIL: bin/install succeeded on old Ruby — the version gate did not fire"
    exit 1
  fi
  if echo "$out" | grep -qiE 'need exactly 3\.4\.10|version mismatch|too old'; then
    echo "OK: bin/install failed with a clear Ruby-version message (exit $status)"
  else
    echo "FAIL: bin/install failed (exit $status) but not with the expected version message"
    exit 1
  fi
}
