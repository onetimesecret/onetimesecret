  ...
  describe "When Onetime.boot! completes" do
    ...
    it "sets OT.d9s_enabled based on configuration" do
      ...
      if Onetime.conf&.dig(:diagnostics, :enabled) &&
        !Onetime.conf&.dig(:diagnostics, :dsn).to_s.strip.empty?
    ...
    context "regarding i18n" do
      ...
      it "initializes i18n settings from config" do
        Onetime.boot!(:test)
        expect(Onetime.conf&.dig(:internationalization, :enabled)).to be(true)
        expect(Onetime.conf&.dig(:internationalization, :fallback_locale)).to be_a(Hash)

        expect(Onetime.supported_locales).to match_array(Onetime.conf&.dig(:internationalization, :locales))
