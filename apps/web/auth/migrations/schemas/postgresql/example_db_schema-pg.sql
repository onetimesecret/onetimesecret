-- ================================================================
-- Rodauth PostgreSQL Database Schema (Context7 Optimized)
-- Authentication and Account Management System
-- Based on official Rodauth documentation and best practices
-- ================================================================
...
-- Uses citext for case-insensitive email handling (PostgreSQL best practice)
...
-- Unique index with status filtering (PostgreSQL partial index for active accounts only)
CREATE UNIQUE INDEX idx_accounts_email_active ON accounts(email) WHERE status_id IN (1, 2);
...
-- Essential indexes for common queries (email index handled by unique constraint above)
...
COMMENT ON TABLE accounts IS 'Core user accounts table with citext email for case-insensitive logins';
COMMENT ON TABLE account_statuses IS 'Account status lookup (unverified, verified, closed)';
COMMENT ON TABLE account_password_hashes IS 'Current password hashes for accounts (separated for security)';
