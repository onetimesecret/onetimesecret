...
CREATE OR REPLACE FUNCTION rodauth_get_salt(account_id BIGINT)
RETURNS TEXT AS $$
DECLARE
    salt TEXT;
BEGIN
...
CREATE OR REPLACE FUNCTION rodauth_valid_password_hash(account_id BIGINT, hash TEXT)
RETURNS BOOLEAN AS $$
DECLARE
    valid BOOLEAN := FALSE;
BEGIN
...
BEGIN
    ...
    DELETE FROM account_password_reset_keys WHERE deadline < CURRENT_TIMESTAMP;

    -- Clean up expired remember keys
    DELETE FROM account_remember_keys WHERE deadline < CURRENT_TIMESTAMP;

    -- Clean up expired lockouts
    DELETE FROM account_lockouts WHERE deadline < CURRENT_TIMESTAMP;

    -- Log cleanup action
    RAISE NOTICE 'Cleaned up expired tokens at %', CURRENT_TIMESTAMP;
END;
$$ LANGUAGE plpgsql;
...
SELECT
    ...
    a.email,
    CASE
        WHEN ask.last_use + INTERVAL '30 days' > CURRENT_TIMESTAMP THEN 'Active'
        ELSE 'Expired'
    END as session_status,
    EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - ask.last_use))/3600 as hours_since_use
...
WHERE last_use < CURRENT_TIMESTAMP - INTERVAL '30 days';
