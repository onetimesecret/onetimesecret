...
module Auth::Config::Hooks
  ...
  module MFA
    def self.configure(auth) # rubocop:disable Metrics/PerceivedComplexity
      ...
      # ========================================================================
      # NOTE: after_two_factor_authentication (the completion hook for ANY
      # second factor — OTP, recovery code, or WebAuthn passkey) is owned by
      # Hooks::TwoFactor (hooks/two_factor.rb), NOT this module. This module
      # only loads when AUTH_MFA_ENABLED=true, but webauthn-only deployments
      # (AUTH_WEBAUTHN_ENABLED=true, AUTH_MFA_ENABLED=false) need the
      # completion hook too — see the ownership note in that file.
      # ========================================================================
