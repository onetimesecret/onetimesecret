...
module Auth::Config::Hooks
  module OmniAuthConnect
    # Release gate, not an operator bypass. Tenant provider suppression in
    # sso-link-evidence.ts and the complete #3849 regression matrix must ship
    # before this becomes true. The authorization pipeline remains mandatory.
    ...
    module Callback
      ...
      def resolve_omniauth_connect_account
        ...
        matching                  = intent.is_a?(Hash) && intent['surface'].is_a?(Hash) &&
