...
RSpec.describe 'Auth::Config::Features::OmniAuth provider registration' do
  ...
  before(:all) do
    ...
    # Provide Onetime.auth_config for the local-IdP provider gate. The real
    # Onetime module is already loaded by the wider spec environment; just
    # ensure auth_config exists. Tests use allow(...).to receive(...) to
    # control oauth_enabled? per-example.
    unless defined?(Onetime)
      module ::Onetime
        def self.auth_config
          @auth_config ||= Object.new
        end
      end
    end
