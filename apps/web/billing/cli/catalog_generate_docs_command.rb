...
module Onetime
  module CLI
    ...
    # Auto on dev:     chained from `predev` (runs once per `pnpm dev` start)
    # Auto on build:   chained from `prebuild` (runs once per `pnpm build`)
    #
    ...
    # an informational line — it is NOT an error. This lets the predev /
    # prebuild hooks run unconditionally without breaking dev environments
    # that don't carry a billing config.
