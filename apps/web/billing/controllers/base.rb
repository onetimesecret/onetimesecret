...
module Billing
  module Controllers
    module Base
      ...
      #
      # Request-side wrapper over Billing::SubscriptionGuard, which owns the
      # rules (and the missing-key / Stripe-error fail-safe) so the request-less
      # checkout path — Operations::CreateCheckoutLink, used by the colonel
      # endpoint and the CLI — applies exactly the same guard. Each caller
      # chooses how to respond: 409 JSON here, a redirect in
      # Plans#checkout_redirect, a :failed result in the op.
      #
      ...
      def org_has_blocking_active_subscription?(org)
        ::Billing::SubscriptionGuard.blocking_active_subscription?(org, logger: billing_logger)
      end
