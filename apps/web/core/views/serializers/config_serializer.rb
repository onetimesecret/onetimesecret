  ...
  module Views
    ...
    module ConfigSerializer
      ...
      def self.serialize(view_vars)
        ...
        output['secret_options'] = build_secret_options(site, view_vars)
      ...
      class << self
        # Publish the guest ceiling the server enforces for this request host.
        #
        # The legacy key name is authentication-centric, but the value is
        # intentionally request-boundary-aware:
        #
        # - canonical host: TTL_MAX_ANONYMOUS (7 days by default)
        # - custom host: owning organization's plan lifetime (normally 14/30d)
        #
        # A branded custom-domain guest is unauthenticated but not unaccountable
        # platform traffic; the tenant owns that storage boundary. Keep this on
        # SecretLifetimePolicy with API enforcement so the dropdown and server
        # cannot disagree again.
        #
        # @param site [Hash] site config section
        # @param view_vars [Hash] request-aware view variables
        # @return [Hash, nil] secret_options for the bootstrap payload
        def build_secret_options(site, view_vars)
          ...
          ceiling     = Onetime::SecretLifetimePolicy.guest_ceiling(
            ...
            domain_strategy: view_vars['domain_strategy'],
            display_domain: view_vars['display_domain'],
          )

          secret_options.merge('ttl_max_anonymous' => ceiling)
        end
