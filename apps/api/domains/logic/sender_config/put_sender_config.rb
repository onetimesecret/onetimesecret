    ...
    module SenderConfig
      ...
      class PutSenderConfig < Base
        ...
        def auto_provision_if_needed
          ...
          if result.success?
            OT.ld "[PutSenderConfig] Auto-provisioned DNS records for #{@domain_id}"
            log_sender_audit_event(
            ...
            OT.lw "[PutSenderConfig] Auto-provisioning skipped/failed for #{@domain_id}: #{result.error}"
          end
        rescue StandardError => ex
          # Don't fail the save if provisioning fails
          OT.le "[PutSenderConfig] Auto-provisioning error for #{@domain_id}: #{ex.message}"
        end
      end
    end
  end
