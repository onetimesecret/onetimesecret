    ...
    module SenderConfig
      ...
      class PatchSenderConfig < Base
        ...
        def auto_provision_if_needed
          ...
          if result.success?
            OT.ld "[PatchSenderConfig] Auto-provisioned DNS records for #{@domain_id}"
            log_sender_audit_event(
            ...
            OT.lw "[PatchSenderConfig] Auto-provisioning skipped/failed for #{@domain_id}: #{result.error}"
          end
        rescue StandardError => ex
          # Don't fail the save if provisioning fails
          OT.le "[PatchSenderConfig] Auto-provisioning error for #{@domain_id}: #{ex.message}"
        end
      end
    end
  end
