...
# Narrow coverage for two properties of the detail payload's `record` hash. The
...
#
#   1. subscription_period_end is a STRING. Organization stores it as an epoch
#      Integer, the colonel console parses this payload with a Zod schema that
...
#      operator cannot tell "one org is broken" from "every org is broken" —
#      exactly the question item 1 raised. The frontend lifts the ref off the
#      RAW payload (a parse failure leaves no parsed record) and attaches it as
#      a tag for enrolled internal/admin schemas only.
#
#      The examples below pin what the privacy posture depends on: the ref is
#      opaque and derived, it is NOT the objid or extid it comes from, and
#      adding it removed none of the identifiers an operator legitimately reads
#      off this authenticated colonel response.
#
# Run with:
...
RSpec.describe ColonelAPI::Logic::Colonel::GetOrganizationDetail do
  ...
  describe 'subscription_period_end' do
    context 'when the org stores an epoch Integer' do
      let(:period_end) { 1_767_225_600 }

      it 'serializes it as a String so the client schema parses' do
        expect(record[:subscription_period_end]).to eq('1767225600')
      end
    end

    context 'when the org has no period end' do
      let(:period_end) { nil }

      it 'stays nil rather than becoming an empty string' do
        expect(record[:subscription_period_end]).to be_nil
      end
    end
  end
