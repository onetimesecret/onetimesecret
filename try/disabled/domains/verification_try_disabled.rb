...
Onetime::CustomDomain.create(@existing_domain, @customer.custid) # Ensure the existing domain is created and added to values

## Can successfully create a custom domain with a valid domain name
begin
  custom_domain = Onetime::CustomDomain.create(@valid_domain, @customer.custid)
...
begin
  custom_domain = Onetime::CustomDomain.create(@invalid_domain, @customer.custid)
rescue OT::Problem => e
  e.message
end
...
custom_domain = Onetime::CustomDomain.create(@existing_domain, @customer.custid)
#=!> OT::Problem

## Can generate TXT validation record for a custom domain
custom_domain = Onetime::CustomDomain.new(@valid_domain, @customer.custid)
...
custom_domain = Onetime::CustomDomain.create(domain_to_delete, @customer.custid)
...
custom_domain = Onetime::CustomDomain.create(domain_to_destroy, @customer.custid)
custom_domain.destroy!(@customer)
...
cd2 = Onetime::CustomDomain.create(conflicting_domain, @other_customer.custid)
[
...
custom_domain = Onetime::CustomDomain.create(apex_domain, @customer.custid)
custom_domain.apex?
#=> true

## Testing apex? method for subdomains
subdomain = "sub.example.com"
custom_subdomain = Onetime::CustomDomain.create(subdomain, @customer.custid)
custom_subdomain.apex?
#=> false

## derive_id generates consistent IDs for the same domain and customer
custom_domain1 = Onetime::CustomDomain.new(@apex_domain, @customer.custid)
id1 = custom_domain1.derive_id
custom_domain2 = Onetime::CustomDomain.new(@apex_domain, @customer.custid)
id2 = custom_domain2.derive_id
id1 == id2
#=> true

## derive_id generates different IDs for different domains or customers
custom_domain3 = Onetime::CustomDomain.new(@apex_domain, @customer.custid)
custom_domain3b = Onetime::CustomDomain.new(@apex_domain, @other_customer.custid)
...
## derive_id generates different IDs for different domains or customers
custom_domain4 = Onetime::CustomDomain.new(@valid_domain, @other_customer.custid)
custom_domain4b = Onetime::CustomDomain.new(@apex_domain, @other_customer.custid)
...
custom_domain_exist = Onetime::CustomDomain.create(existing_domain, @customer.custid)
custom_domain_exist.exists?
#=> true

## exists? method returns false for non-existing custom domains
non_existing_domain = Onetime::CustomDomain.new("non-exist-#{SecureRandom.hex(4)}.example.com", @customer.custid)
non_existing_domain.exists?
#=> false

## Attempting to load a non-existing custom domain raises RecordNotFound
begin
  Onetime::CustomDomain.load("non-existent-domain.example.com", @customer.custid)
...
max_depth_subdomain = ('a.' * (Onetime::CustomDomain::MAX_SUBDOMAIN_DEPTH - 2)) + 'example.com'
custom_domain_depth = Onetime::CustomDomain.create(max_depth_subdomain, @customer.custid)
custom_domain_depth.display_domain == max_depth_subdomain
#=> true

## Exceeding maximum subdomain depth raises an error
too_deep_domain = ('a.' * Onetime::CustomDomain::MAX_SUBDOMAIN_DEPTH) + 'example.com'
begin
  Onetime::CustomDomain.create(too_deep_domain, @customer.custid)
rescue OT::Problem => e
  e.message
end
...
max_length_domain = ('a' * (Onetime::CustomDomain::MAX_TOTAL_LENGTH - 11)) + '.com'
custom_domain_length = Onetime::CustomDomain.create(max_length_domain, @customer.custid)
custom_domain_length.display_domain == max_length_domain
#=> true

## Exceeding maximum total domain length raises an error
too_long_domain = ('a' * Onetime::CustomDomain::MAX_TOTAL_LENGTH) + '.com'
begin
  Onetime::CustomDomain.create(too_long_domain, @customer.custid)
rescue OT::Problem => e
  e.message
end
...
success = Onetime::CustomDomain.default_domain?('default.example.com')
...
new_conf = { 'site' => { 'host' => 'default.example.com' } }
OT.instance_variable_set(:@conf, new_conf)
success = Onetime::CustomDomain.default_domain?('non-default.example.com')
OT.instance_variable_set(:@conf, old_conf)
success
#=> false
