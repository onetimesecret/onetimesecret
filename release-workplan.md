...
6. **Add ordered snapshot ingestion.** Add `snapshot_generated_at` to every complete bootstrap payload and pass it through `apps/web/core/views/serializers/system_serializer.rb`, `src/schemas/contracts/bootstrap.ts`, `src/shared/stores/bootstrapStore.ts`, and `src/services/bootstrap.service.ts`.
   - Use fixed-width UTC RFC 3339 with exactly six fractional digits, for example `2026-09-17T17:28:59.123456Z`.
   - Generate it once per server snapshot, after authentication resolution, and emit it through both HTML hydration and `GET /bootstrap/me`.
   - Validate the exact format in the shared Zod/Rhales contract and compare canonical strings directly; JavaScript `Date` conversion would discard microsecond precision.
   - The hydrated snapshot establishes the initial accepted watermark. A later complete snapshot is eligible only when its timestamp is strictly newer.
   ...
   - Ignore equal, older, malformed, or—after a timestamped snapshot has been accepted—unversioned responses without partially updating any store. Local state patches never advance the watermark.
   - Treat equal timestamps with different payloads as an invariant violation: retain the accepted state, emit structured diagnostics, and schedule one coordinated retry.

   A datetime alone cannot guarantee total ordering across equal timestamps, clock rollback, or unsynchronized workers. Deterministic client behavior therefore also requires one refresh coordinator, monotonically increasing client request generations, and acceptance only when both the server timestamp is newer and the request generation remains current. Record clock regressions and retain the last accepted state. If the supported release topology cannot establish a sufficiently strong server-clock invariant, add a server-authoritative monotonic `snapshot_version` in v0.26.13 rather than claiming the datetime alone is sufficient.
...
8. **Unify refresh and navigation.** Route guards, masthead actions, scheduled checks, and protected-API reconciliation use one refresh coordinator. Refresh only after authentication mutations such as login, logout, MFA completion, impersonation, or account switching; when returning to a stale visible tab; at the passive verification interval; after a rejection requiring reconciliation; or on explicit retry.
   ...
   The coordinator must deduplicate equivalent requests, allow at most one ordinary refresh in flight, assign request generations to exceptional overlaps, and cancel or invalidate obsolete generations. Authentication mutations invalidate older refresh generations. A response may be ingested only when its generation is still current and its snapshot timestamp is newer than the accepted watermark. An older response must never restore identity after a later rejection, logout, or account change. A server-verified hydrated snapshot can satisfy initial navigation; rejected, checking, or unavailable state cannot redirect `/signin` to Dashboard using cached authentication.

9. **Handle rejection and unavailability distinctly.** A definitive session rejection atomically clears protected client state and opens the appropriate sign-in or MFA flow. Network failure or unverifiable authority blocks protected content and actions, preserves the server session and last accepted snapshot internally, and shows a retryable verification-unavailable view without exposing protected UI from stale state. Remove automatic logout caused solely by repeated transport failures. Keep admin-expiry recovery scoped to the admin surface. Protected API rejection requests reconciliation through the coordinator rather than directly mutating authentication state.
...
- Add `snapshot_generated_at` to every complete hydrated or fetched server snapshot under the ordering contract above.
...
- Coordinate backend/frontend deployment. Legacy payloads with `authenticated: false` may only withhold access; no compatibility path may promote them to authenticated. Once a timestamped snapshot has been accepted, an unversioned payload cannot mutate state.
...
| Add a centralized monotonic `snapshot_version` if server timestamps are operationally adequate but not definitive | Strengthens cross-worker ordering without delaying v0.26.13 when its supported topology has a proven clock invariant.  |

Promote `snapshot_version` into v0.26.13 if multiple unsynchronized workers are supported in the release configuration. Likewise, a reproducible session-fixation, authorization-bypass, or credential-disclosure issue discovered during v0.26.13 assessment is promoted into the current release. Version placement must change when the evidence changes.
...
- Verify the exact `snapshot_generated_at` format in hydrated and fetched payloads. Prove differences below one millisecond remain ordered without `Date` conversion.
...
- Ordering tests prove newer snapshots replace state; equal, older, malformed, and disallowed unversioned responses cannot mutate it; reverse-resolution order retains the newest eligible snapshot; local partial patches do not advance the watermark; and authentication mutations invalidate late requests.
...
- During staging and rollout, review refusal codes, redirect loops, verification-unavailable rates, clock-regression diagnostics, stale-snapshot rejection, and bootstrap query/write behavior. Unexpected revocations or increased activity writes reopen the relevant assumption before broader rollout.

Assumptions: this plan categorizes the authentication work discussed here, not the entire release backlog. v0.26.13 may contain the larger shared-design change, as selected. Retryable blocking is the chosen outage behavior because it preserves recovery while withholding protected access when verification is unavailable. Timestamp ordering is accepted only for release topologies with an established clock invariant; otherwise `snapshot_version` is part of the v0.26.13 release gate.
