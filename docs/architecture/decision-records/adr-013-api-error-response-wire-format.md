...
- `request_id` is a cross-cutting correlation field (not class-specific). Both surfaces echo the request's `x-request-id` into every typed error body through one shared helper, `Onetime::Application::ErrorCorrelation` (invoked by `OttoHooks#with_error_correlation` on the Otto apps and by the Roda `:error_handler` on the `/auth` surface), so an API consumer who reports an error gives us an id that appears verbatim in the `RequestLogger` request-log line (which logs `request_id` alongside the `error_type` the helper stashes in the Rack env). It is informational only — the frontend does not branch on it. The request-independent fallbacks omit it from the body — the Otto `router.not_found`/`router.server_error` responses and the auth router's static 404 (`Auth::ErrorTranslator::NOT_FOUND_BODY`, returned from routing rather than raised) — and for those the `x-request-id` response header still carries the id. This supersedes the Otto-minted `error_id`, which is development-only in the body and logged without the `request_id`.
