...
| **destroy** | Delete a Rack session's blob from Redis. Logout does this; so do the colonel session operations. Ends the session regardless of the authdb. The `/auth` router also destroys a Rack session whose row the gate reports `:revoked`, before routing, the way it already handles an orphaned session: the request is then refused with the `session_expired` key, except login and logout, which continue anonymously so a stale cookie can sign in again. |
| **refuse** | Answer a request with 401 (Otto auth strategies, `/auth` router) or `authenticated? == false` (`SessionHelpers`) while leaving the Rack session in Redis. The only thing `Onetime::ActiveSessionGate` does. A refused Rack session is replaced by the next login, or honoured again if it was refused only because its row could not be checked. |
