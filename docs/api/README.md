...
These conventions apply to secret and receipt responses. Field value _types_ differ by version (see [API Versions](#api-versions)); the field _meanings_ below are the same across versions unless noted.
...
- Authenticated callers are governed separately. A free-tier request above 14 days is _rejected_ with an entitlement error rather than clamped, so the caller gets an explicit upgrade path instead of a shortened secret.
