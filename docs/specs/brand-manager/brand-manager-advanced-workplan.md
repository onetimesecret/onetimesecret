...
**C6 — Dark-mode resolution + background/text color consumers** (1.5d, deps: C14; Q1 RESOLVED 2026-07-11 — unblocked)
brandbg/brandtext are single-value light-only tokens (style.css:125-126) vs a live theme toggle. DECIDED (Q1): operator-explicit values win both schemes — conditional class swap to `bg-brandbg`/`text-brandtext` without `dark:` when set; unset domains pixel-identical. The editor's existing WCAG contrast advisory is the guardrail (C7). **Placement fix vs draft:** BaseSecretDisplay's swap derives from **props.domainBranding** in a local computed (props-only component, reused by SecretPreview for the EDITED record — an identityStore conditional would key preview set-ness on the wrong domain); the identityStore computed (hasSurfaceColors/brandSurfaceClasses, mirroring cornerClass at identityStore.ts:296-307) serves only store-reading surfaces (BrandedHomepage). Wire: BaseSecretDisplay :62,:97,:107,:145-146; BrandedHomepage h1 :125, subline :128, trust-card text AND background :187 (draft wired text only — homepage had zero background surface); SecretForm card :152 gets the operator background too (Q1b resolved with Q1 — "recipients see the colors the operator picked" applies to every branded surface); extend C13's unknown-secret surfaces with the same prop-derived swap. (Derived-dark-pair alternative rejected with Q1; the EXTENDED_KEYS/clearOverrides caveat is moot.) No new safelist entries (tailwind-safelist.ts:43-50 covers these; template classes static).
...
Colors in light AND dark (the core Q1 risk); unset domains pixel-identical; preview-vs-recipient parity including the cross-domain case (editing domain B while operator's domain has no colors — the C6 regression case). The former known-gaps (DisabledClosed/Q5, ShowSecret Q8 snapshot) were fixed 2026-07-11 — verify them as regular surfaces, not exclusions.
...
- Q1/Q2 stall contingency (moot as of 2026-07-11 — both resolved): Waves 1-2 + C14 ship a complete, **browser-verified** Advanced v1 (heading font + full-range radius, covering homepage, reveal, unknown-secret, and disabled surfaces) to main in ~6 days.
...
2. **Q2 — RESOLVED 2026-07-11: supporting-accent role.** brand2 owns the soft/decorative accents (icon chips, DisabledV1 eyebrow dot/promo chip, BaseSecretDisplay once-only footer marker); primary keeps buttons, links, and focus states (interactive-state role rejected — focus contrast a11y risk). C8 unblocked.
...
5. **Q5 — RESOLVED 2026-07-11: wire.** Even when the homepage is disabled for a custom domain, it must represent the brand. DONE on fix/brand-token-consumers: DisabledClosed threads headingClass/fontClass into DisabledHomepageTaglines (background color rides C6 like every other surface). Flip-default and accept-the-gap rejected.
