...
- Extract `Operations::Customers::{List, Show, SetRole, SetVerification, Purge, Doctor}` from today's inline CLI logic. Both `bin/ots customers *` and NEW colonel Logic classes delegate to them.
  ...
  - `Purge` — fold CLI's duplicate `delete_customer_keys` into the existing `Auth::Operations::DestroyCustomerRecord`.
...
- [ ] `SetVerification` reuses existing `Auth::Operations::SetCustomerVerification` (not a rewrite); Purge reuses `Auth::Operations::DestroyCustomerRecord`.
