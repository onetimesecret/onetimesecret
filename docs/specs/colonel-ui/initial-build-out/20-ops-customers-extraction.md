...
- Extract `Operations::Customers::{List, Show, SetRole, SetVerification, Purge, Doctor}` from today's inline CLI logic. Both `bin/ots customers *` and NEW colonel Logic classes delegate to them.
  ...
  - `Purge` — fold CLI's duplicate `delete_customer_keys` into the existing `Auth::Operations::DeleteCustomerRecord`.
...
- Incumbent ops home: `apps/web/auth/operations/` — `set_customer_verification.rb`, `delete_customer_record.rb`, `ensure_customer_for_account.rb`, `close_account.rb`, `bulk_sso_migration.rb` (+ `apps/web/billing/operations/`).
...
- Delete duplication: `Auth::Operations::DeleteCustomerRecord` (`apps/web/auth/operations/delete_customer_record.rb`) vs CLI purge's own `delete_customer_keys` (`lib/onetime/cli/customers/purge_command.rb:265,555`).
...
- [ ] `SetVerification` reuses existing `Auth::Operations::SetCustomerVerification` (not a rewrite); Purge reuses `Auth::Operations::DeleteCustomerRecord`.
