...
| `ip_masked`, `asn`, `geo_country`, `device_label`, `flagged_at`, `flag_reason` | Raw `ip_address`, `user_agent` copied **as-is**; geo/asn/device/flag fields dropped at the time    | Otto already masks IP/UA upstream, so no masking logic is built here. Geo/ASN enrichment and anomaly-flagging were out of scope for the first cut. **Update (#3989):** `geo_country` is no longer dropped — see the follow-up note below. `asn`, `device_label`, `flagged_at`, `flag_reason` remain out of scope; otto has no ASN support at all.                                                                                                                                                                                                               |
...
| revoke-all → `rodauth.remove_all_active_sessions_for`                           | revoke-all clears `account_active_session_keys` via **direct Sequel** (full mode only) | The ops layer has no bound Rodauth instance, so it can't call the helper — it does the same DELETE `RemoveAuthenticationData` does, resolving `account_id` from `accounts.external_id`. Guarded on the auth DB being present (nil in simple mode → skipped).                                                                                                                   |
...
- **`geo_country` is now populated (#3989)** — otto 2.8 added country-level geo resolution (`Otto::Privacy::GeoResolver`, enabled by default) and stamps `env['otto.privacy.geo_country']` on every request: a trusted CDN header (e.g. Cloudflare `CF-IPCountry`) or a masked-IP MMDB lookup, `'**'` when unresolved, never a raw-IP lookup. `TrackMetadata` now reads that key and persists `geo_country` on the sidecar, same best-effort/no-op-on-failure discipline as the other fields. **ASN and device fingerprinting are still out of scope** — otto has no ASN resolver, and this change does not add one. Legacy sessions minted before this change carry nil `geo_country` until they churn. See `../geo-precision-profile-evaluation.md` for why this does not require loosening the /24–/48 IP-masking posture.
...
- **Current-session indicator — BUILT.** `ListCustomerSessions` returns `details.current_session_id` (the acting colonel's own request sid, via `safe_session_id.public_id`). When it matches a listed row (colonel viewing their own detail), the panel badges that row "This session" and disables its per-row revoke instead of offering a no-op button: revoking your **own** current session is a no-op because Rack's `write_session` re-persists the blob at the end of the same request. Revoke-all is unaffected. The id is returned whenever the acting session can be identified — even when viewing another customer's sessions, where it won't match any listed row (the UI does the row matching). Nullable/absent only when the current session can't be identified (e.g. a Hash session under JSON auth).
...
Population happens in `Onetime::Session#write_session` (not a Rodauth hook — see the table) via `Operations::Sessions::TrackMetadata`, best-effort with its own rescue, refreshing `last_activity_at` and the sorted-set score on each authenticated write. Geo/ASN/device enrichment was dropped from the first cut. **Update (#3989, otto 2.8):** `env['otto.privacy.geo_country']` is now available — `Otto::Privacy::GeoResolver` resolves an ISO 3166-1 alpha-2 country code (or the `'**'` sentinel when unknown) from a trusted CDN header or a masked-IP MMDB lookup, never the raw IP — and `TrackMetadata` now reads it and persists `geo_country` on the sidecar. ASN and device enrichment remain out of scope: otto has no ASN support, and there is still no fingerprint collection. See `../geo-precision-profile-evaluation.md` for why this does not require loosening the /24–/48 masking posture.
