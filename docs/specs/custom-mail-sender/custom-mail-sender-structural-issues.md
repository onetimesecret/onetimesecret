...
- Required-credential knowledge lives only in ProvisionSenderDomain.missing_credential_keys. DomainValidationWorker guards on credentials.empty?, which YAML defaults make impossible → a rotated key silently flips verified domains to failed (lettermint and smtp2go both).
