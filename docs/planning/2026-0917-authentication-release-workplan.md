...
6. **Add ordered snapshot ingestion per ADR-046.** (#4457 server allocation, #4464 client acceptance, #4465 forced page load and `beforeunload` guard) ADR-046 (Bootstrap State Ordering Contract) is normative for field definitions, server allocation, client acceptance rules, failure behavior, and the test list. This plan does not restate them; where this plan or a sub-issue differs from the ADR, the ADR wins. The outcomes this release depends on:
   - Complete snapshots for ordered sessions carry `snapshot_epoch` and `snapshot_version` through both HTML hydration and `GET /bootstrap/me`; anonymous and otherwise unordered snapshots omit both. Those two fields and the client request generation decide order. `snapshot_generated_at` is diagnostic-only and never decides whether a snapshot is applied.
...
8. **Unify refresh and navigation.** (#4459) Route guards, masthead actions, scheduled checks, and protected-API reconciliation use one refresh coordinator. Refresh only after authentication mutations such as login, logout, MFA completion, impersonation, or account switching; when returning to a stale visible tab; at the passive verification interval; after a rejection requiring reconciliation; or on explicit retry.
   ...
   The coordinator must deduplicate equivalent requests, allow at most one ordinary refresh in flight, give every request for a complete snapshot the next request generation, and cancel or invalidate obsolete generations. A failed refresh is retried with exponential backoff and jitter, honoring `Retry-After`. Authentication mutations invalidate older refresh generations. A response may be ingested only when its generation is still current and it passes the ADR-046 epoch and version acceptance rules. An older response must never restore identity after a later rejection, logout, or account change. A server-verified hydrated snapshot can satisfy initial navigation; rejected, checking, or unavailable state cannot redirect `/signin` to Dashboard using cached authentication.

9. **Handle rejection and unavailability distinctly.** (#4460) Only a definitive server statement ends the client's authenticated state: a snapshot that reports no session, or a session-scoped authentication failure code. Handle a no-session snapshot according to its request type under ADR-046. On the current authentication-mutation generation, accept it atomically, retire the prior epoch, clear protected client state, and open the appropriate sign-in or MFA flow. On an ordinary refresh, do not apply it in place or clear account-scoped state piecemeal; record the diagnostic, stop ordinary refreshes, enter stale-session state, and take the forced-page-load path. A transport failure, timeout, or 5xx response, including the ADR's allocation-failure 503, is not an authentication verdict. The coordinator retains the last accepted snapshot and retries. At the consecutive-failure limit the client enters `unavailable`: protected content and actions are blocked, a retryable verification-unavailable view is shown without exposing protected UI from stale state, the server session is untouched, and retries continue at a capped interval. The first successful refresh processes whatever the server reports under the ADR-046 acceptance and forced-page-load rules. Remove the client-only logout at `MAX_FAILURES` in `authStore.checkWindowStatus`. Keep admin-expiry recovery scoped to the admin surface. Protected API rejection requests reconciliation through the coordinator rather than directly mutating authentication state.

   Basis for not logging out at the limit:
   - A failed request says nothing about the credential. RFC 9110 defines 401 as lacking valid authentication credentials and 503 as a temporary inability to handle the request. OAuth 2.0 (RFC 6749) separates `invalid_grant` from `temporarily_unavailable` the same way, and MSAL.js requires re-authentication only for `InteractionRequiredAuthError` while telling applications to retry network and overload failures.
   - The OWASP Session Management Cheat Sheet requires expiry to be enforced server-side and logout to invalidate the session server-side. Today's limit calls `authStore.logout()`, which clears client state only. The page shows a signed-out user while the cookie still authenticates, and a reload restores the session. The server is unreachable at that moment, so a real logout is not available, and destroying a valid session because of an outage would be a self-inflicted availability failure.
   - Fail-safe defaults (Saltzer and Schroeder; OWASP "fail securely") call for withholding access when authority cannot be established. That is the `unavailable` state: block protected UI, claim nothing about identity.
   - Retry guidance (AWS Builders' Library, "Timeouts, retries, and backoff with jitter") is backoff with jitter. Logging every open tab out during a transient outage sends users to the login endpoint, the most expensive authentication path, while the backend is degraded.
...
- Add `snapshot_epoch` and `snapshot_version` as a validated pair under ADR-046: both are present and valid for a complete snapshot of an ordered session, or both are absent. Anonymous or otherwise unordered snapshots and degraded hydration payloads omit the pair. Normal ordered server snapshots also include `snapshot_generated_at`, but it is independently optional at the client boundary and diagnostic-only; a missing or malformed timestamp reports unknown age and a diagnostic without rejecting an otherwise valid snapshot.
...
- Verify anonymous, MFA-pending, checking, and unavailable transitions remove or block protected customer data as required. Temporary verification failure must block protected UI without destroying the server session, and no number of transport failures or 5xx responses logs the user out. Failed refreshes back off with jitter and honor `Retry-After`.
