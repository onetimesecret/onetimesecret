...
**Verdict:** build tokens-for-neutrals and the atomic primitives _inside_
...
1. **Semantic neutral + status tokens** in `src/assets/style.css` `@theme` block:
   ...
   them later. Treat the token _names_ as stable but the gray-vs-`brand2` backing as
...
- Merge secret's `canonical/` vs `branded/` component pairs into single
  ...
  pipeline solves the _theming_ half, but the two trees also share the
