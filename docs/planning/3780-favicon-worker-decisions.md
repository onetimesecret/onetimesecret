...
| 4   | Fetched-icon vs user-uploaded-logo precedence       | **Only protect uploaded icon.** Auto-fetch may populate `icon` even when a user-uploaded logo exists; a fetched favicon then outranks the logo _as the favicon_. (The upload-overwrite guard still protects a user-uploaded **icon**.) |
...
- **Phase 1 (core):** SafeFetch + operation + worker + queue/publisher + storage/serving + config + trigger-on-verify + tests. _(this workflow)_
