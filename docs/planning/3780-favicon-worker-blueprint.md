...
**GetFavicon precedence is NOT changed** for the strict AC. The only writer that could clobber user content is the worker (guarded above); `GetFavicon#generate_and_cache_favicon` only writes the derived `encoded_favicon` cache, never `encoded`/`filename`, so it cannot clobber a user icon. Whether a _fetched icon_ should outrank a _user-uploaded logo_ in `raise_concerns` (icon>logo, L54-67) is a product call — open question 4.
...
4. **Fetched-icon vs user-uploaded-logo precedence.** GetFavicon serves `icon` over `logo` (L54-67). After this feature, a _fetched_ icon will outrank a user's _uploaded logo_. AC only protects the uploaded _icon_; is overriding the logo acceptable, or should `raise_concerns` prefer a `user_upload` logo over an `auto_fetch` icon? (Map 3 open Q1.)
