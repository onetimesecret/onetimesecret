...
- **Federated subscription claim without email verification.** `apps/web/auth/operations/ensure_default_workspace.rb:314-333` carries an explicit "SECURITY AUDIT (verify-disabled residual)" block that logs loudly rather than blocking, on the stated grounds that blocking would silently disable federation for verify-disabled deployments. Deliberate, annotated, unchanged in this window.
