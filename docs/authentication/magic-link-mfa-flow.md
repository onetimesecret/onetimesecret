...
## User Journey: MFA Recovery

**Note:** For complete MFA recovery documentation including console operations and support processes, see `docs/authentication/mfa-recovery.md`.

### When Users Get Locked Out

Users can become locked in MFA verification in several scenarios:

1. **Lost/Broken Authenticator Device**: Phone lost, broken, or reset
2. **Invalidated OTP Keys**: Server-side MFA configuration changes
...
- OTP codes don't work
- Recovery codes unavailable
- **Requires recovery action**

### Primary Recovery Method: Recovery Codes

**User Flow:**
1. User stuck at MFA verification screen
2. Clicks "Use a recovery code instead"
3. Enters one of their saved recovery codes
4. Access granted immediately
5. User should generate new recovery codes from settings

**Best Practice:** Save recovery codes when setting up MFA in a password manager or secure physical location.

### Admin/Support Recovery

For users who lost both authenticator and recovery codes, administrators can disable MFA via console:
...
This removes all MFA setup and allows the user to log in with email/password only.

**See:** `docs/authentication/mfa-recovery.md` for complete admin and support recovery procedures.
...
- **Backup Authenticators**: Allow users to register multiple TOTP devices
- **WebAuthn Primary**: Support passwordless WebAuthn as primary authentication method
