...
```bash
python locales/scripts/tasks/create.py eo --levels
```
This populates the `level_tasks` table required by `tasks/next.py`. Run once per locale, or re-run to refresh after English source changes.
...
locales/scripts/
  ...
  tasks/
    create.py         # --levels populates level_tasks for workflow
