...
if worker_count.positive?
  ...
  # Hook naming: Puma 6 uses `on_worker_boot`; Puma 7 renamed it to
  # `before_worker_boot` as part of a broader on_* → before_*/after_* convention
  # change. This project pins to ~> 6.6 (see Gemfile), so we use the Puma 6 name.
  # If upgrading to Puma 7, rename this and audit: on_restart, on_worker_shutdown,
  # on_booted, on_thread_start, on_thread_exit, on_refork → before_*/after_* forms.
  # Also note Puma 7 lowercases response headers, which can break case-sensitive
  # middleware reads.
  on_worker_boot do
