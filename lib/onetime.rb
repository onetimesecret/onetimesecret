...
module Onetime
  ...
  module ClassMethods
    ...
    def print_log_banner
      ...
      if site_config.key?(:domains)
        OT.li "domains: #{site_config[:domains].map { |k,v| "#{k}=#{v}" }.join(', ')}"
      end
      if site_config.key?(:regions)
        OT.li "regions: #{site_config[:regions].map { |k,v| "#{k}=#{v}" }.join(', ')}"
      end
