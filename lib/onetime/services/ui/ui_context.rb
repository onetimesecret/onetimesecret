...
module Manifold
  ...
  class UIContext < RSFC::Context
    ...
    def build_onetime_window_data(req, sess, cust, locale_override)
      ...
      add_configuration_data(jsvars, site, secret_options, regions, regions_enabled, incoming_recipient, support_host)
      ...
      if authenticated && cust
        ...
        if domains_enabled
          custom_domains = cust.custom_domains_list.filter_map do |obj|
            ...
            unless obj.ready?
              OT.li "[custom_domains] Allowing unverified domain: #{obj.display_domain} (#{obj.verified}/#{obj.resolving})" if defined?(OT) && OT.respond_to?(:li)
            end
            obj.display_domain
          end
    ...
    def add_diagnostics_data(jsvars)
      ...
      if defined?(Onetime) && Onetime.respond_to?(:with_diagnostics)
