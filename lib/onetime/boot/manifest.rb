...
#
# BootManifest provides structured, numbered progress tracking for the
...
module Onetime
  class BootManifest
    ...
    def initialize(logger = SemanticLogger['Boot'])
      @logger = logger
      @completed = []
      @total = STEPS.size
      @start_time = Time.now
    ...
    def checkpoint(step_key)
      ...
      @logger.info "[#{index + 1}/#{@total}] #{step_name}"
      ...
      @logger.debug "Completed #{step_name} in #{elapsed}ms" if elapsed > 100
    ...
    def complete!
      ...
      @logger.info "Boot sequence complete in #{elapsed}ms"
    end
  end
end
