...
module Onetime
  class AuthConfig
    ...
    def mode
      ...
      auth_config.dig('mode') || 'basic'
    end

    # Rodauth configuration
    def rodauth
      auth_config.dig('rodauth') || {}
    end

    # Basic mode configuration
    def basic
      auth_config.dig('basic') || {}
    end

    # Session configuration
    def session
      session_config = auth_config.dig('session') || {}
    ...
    def load_config
      ...
      @config = default_config
      warn "[AuthConfig] Error loading configuration: #{e.message}, using defaults"
    end
    ...
    def deep_merge(base_hash, override_hash)
      base_hash.merge(override_hash) do |key, base_value, override_value|
