...
`DeleteCustomer`. They are the _single implementation_ of each verb; the colonel
...
single app owns). The test: _if a domain app already owns the model and its
invariants, the op belongs in that app._ When in doubt, prefer app-scoped — moving
