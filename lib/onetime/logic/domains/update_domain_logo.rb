  ...
  module Domains
    class UpdateDomainLogo < OT::Logic::Base
      ...
      def raise_concerns
        ...
        if @logo_file
          raise_form_error "Logo file is too large" if @logo_file.size > 5 * 1024 * 1024  # 5 MB
          raise_form_error "Invalid file type" unless ['image/jpeg', 'image/png', 'image/gif'].include?(@logo_content_type)
        else
