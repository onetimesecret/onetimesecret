# lib/onetime/cli/email_templates_command.rb
#
...
module Onetime
  module CLI
    class EmailTemplatesCommand < Command
      ...
      def show_template_detail(name, format)
        sym = name.to_sym
        unless EmailSendCommand::AVAILABLE_TEMPLATES.include?(sym)
          warn "Unknown template: #{name}"
          warn
          warn 'Available templates:'
          EmailSendCommand::AVAILABLE_TEMPLATES.each { |t| warn "  #{t}" }
          exit 1
        end
      ...
      def build_template_list
        EmailSendCommand::AVAILABLE_TEMPLATES.map do |name|
    ...
    register 'email templates', EmailTemplatesCommand
  end
end
