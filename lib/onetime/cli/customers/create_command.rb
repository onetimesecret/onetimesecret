# lib/onetime/cli/customers/create_command.rb
#
# frozen_string_literal: true

# Create a new customer record with optional role, password, and verification.
#
# Usage:
#   bin/ots customers create user@example.com                   # Default role
#   bin/ots customers create user@example.com --role colonel     # Create admin
#   bin/ots customers create user@example.com --password secret  # Set password

module Onetime
  module CLI
    class CustomersCreateCommand < Command
      desc 'Create a new customer with the specified email address'

      argument :email,
        type: :string,
        required: true,
        desc: 'Email address for the new customer'
      ...
      def call(email:, role: 'customer', password: nil, verified: true, **)
        boot_application!
