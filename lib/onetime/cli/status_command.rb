...
module Onetime
  module CLI
    # Text formatting helpers for status output
    module StatusFormatter
      extend self

      def format_text_status(status, quiet)
        lines = []
      ...
      def process_status_text(name, info)
        case info[:status]
        when 'running'
          running_process_text(name, info)
        when 'stopped'
          stopped_process_text(name, info)
        when 'not_required'
          'not required'
        when 'unknown'
          info[:reason] || 'unknown'
      ...
      def running_process_text(name, info)
        case name
        when :puma, :vite
      ...
      def stopped_process_text(name, info)
        case name
        when :puma, :vite
    ...
    class StatusCommand < Command
      ...
      def display_status(format, quiet)
        ...
        when 'json'
          ...
          puts StatusFormatter.format_text_status(status, quiet)
        end
      end
      ...
      def collect_status
...

        {
          ...
          auth_mode: auth_mode,
          boot: { success: @boot_success, error: @boot_error },
          services: {
            redis: check_redis,
      ...
      def check_auth_database
        ...
        begin
          ...
          version = fetch_db_version(db, adapter)
      ...
      def fetch_db_version(db, adapter)
        case adapter
        when 'sqlite'
      ...
      def check_queue_depths(conn)
        ...
        begin
          Onetime::Jobs::QueueConfig::QUEUES.each_key do |queue_name|
            queues[queue_name] = fetch_queue_info(channel, queue_name)
          end
        ensure
      ...
      def fetch_queue_info(channel, queue_name)
        queue = channel.queue(queue_name, durable: true, passive: true)
        { messages: queue.message_count, consumers: queue.consumer_count }
      ...
      def check_worker(rabbitmq_status)
        return { status: 'not_required', enabled: false } unless jobs_enabled?

        queues          = rabbitmq_status[:queues] || {}
