...
module Onetime
  module Initializers
    module ConfigureDomains

      using IndifferentHashAccess

      def self.run(options = {})
        is_enabled = OT.conf.dig(:site, :domains, :enabled).to_s == "true"
        return unless is_enabled

        cluster = OT.conf.dig(:site, :domains, :cluster)
        ...
        OT.ld "[initializer] Domains configured (enabled: #{is_enabled})"
      end
