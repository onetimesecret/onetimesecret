  ...
  class RedisKeyMigrator
    ...
    def migrate_keys(pattern = '*', &)
      ...
      if should_use_streaming_mode?(pattern)
        ...
        migrate_keys_streaming(pattern, strategy, &)
      else
        ...
        case strategy
        when :copy
          migrate_using_copy_command(keys, &)
        when :migrate
          migrate_using_migrate_command(keys, &)
        when :dump_restore
          migrate_using_dump_restore(keys, &)
        else
    ...
    def normalize_uri(uri)
      port = uri.port || 6379
      "#{uri.host}:#{port}/#{uri.path&.delete('/') || uri.db || 0}"
    end
    ...
    def discover_keys(pattern)
      ...
      loop do
          ...
          if block_given? && should_report_progress?(discovered_count)
            yield(:discovery, discovered_count, nil, nil, nil)
          end

          break if cursor == '0'
      rescue StandardError => ex
          handle_error('Key discovery failed', ex)
          break
      end
      ...
      keys.each_slice(@options[:batch_size]) do |batch|
          ...
          if progress_block
            batch.each_with_index do |key, idx|
              yield(:migrate, migrated_count - batch.size + idx, 'migrate', key, -1)
            end
          end
      rescue StandardError => ex
          handle_batch_error(batch, ex, source_client, nil, &progress_block)
      end
    ensure
      source_client&.disconnect!
    ...
    def migrate_using_copy_command(keys, &progress_block)
      ...
      keys.each_slice(@options[:batch_size]) do |batch|
          ...
          batch.each do |key|
            ...
            if progress_block
              yield(:migrate, migrated_count, 'copy', key, -1)
            end
          end
    ...
    def migrate_using_dump_restore(keys, &progress_block)
      ...
      keys.each_slice(@options[:batch_size]) do |batch|
          ...
          if progress_block
            batch.each_with_index do |key, idx|
              yield(:migrate, migrated_count - batch.size + idx, 'dump_restore', key, ttls[idx])
            end
          end
      rescue StandardError => ex
          handle_batch_error(batch, ex, source_client, target_client, &progress_block)
      end
    ensure
      source_client&.disconnect!
    ...
    def handle_batch_error(batch, error, source_client = nil, target_client = nil, &)
      ...
      if batch.size > 1 && @options[:retry_attempts] > 0
        ...
        retry_partial_batch(batch, source_client, target_client, &)
      else
        # Fall back to individual key migration
        @logger.debug 'Falling back to individual key migration'
        retry_individual_keys(batch, source_client, target_client, &)
      end
    end
    ...
    def retry_partial_batch(batch, source_client = nil, target_client = nil, &)
      ...
      [first_half, second_half].each do |sub_batch|
        ...
        begin
          ...
          @logger.debug "Partial batch retry failed (#{sub_batch.size} keys): #{ex.message}"
          # If partial retry fails, fall back to individual key migration
          retry_individual_keys(sub_batch, source_client, target_client, &)
        end
      end
    end

    # Retry individual keys when batch operations fail
    def retry_individual_keys(batch, source_client = nil, target_client = nil, &)
      batch.each do |key|
          migrate_single_key(key, source_client, target_client, &)
      rescue StandardError => ex
          handle_error("Failed to migrate key #{key}", ex)
    ...
    def should_use_streaming_mode?(pattern)
      ...
      source_client  = create_redis_client(@source_uri)
      _, sample_keys = source_client.scan(0, match: pattern, count: 1000)
      source_client.disconnect!
    ...
    def migrate_keys_streaming(pattern, strategy, &progress_block)
      ...
      loop do
          ...
          handle_error('Streaming migration failed', ex)
          break
      end
