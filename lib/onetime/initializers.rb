...
module Onetime
  module Initializers
    ...
    # Discover and load plugin initializers from apps/{web,api}/.
    #
    # Called by boot! rather than at require time so that CLI commands
    # that never boot (DelayBootCommand subclasses) avoid instantiating
    # config singletons (BillingConfig, AuthConfig) and their file I/O.
    def self.discover_plugins!
