...
module Onetime
  class DomainSsoConfig < Familia::Horreum
    ...
    # O(1) lookup index: domain_id -> domain_id
    class_hashkey :configs_by_domain
    ...
    class << self
      ...
      # @return [DomainSsoConfig, nil] The config or nil if not found
      def find_by_domain_id(domain_id)
        return nil if domain_id.to_s.empty?

        # Check index first (fast path)
        return nil unless configs_by_domain.member?(domain_id)

        # Load the config using domain_id as identifier
        load(domain_id)
      ...
      def exists_for_domain?(domain_id)
        ...
        configs_by_domain.member?(domain_id)
      end
      ...
      # @return [DomainSsoConfig] The created config
      ...
      #
      # @return [Array<DomainSsoConfig>] All configs (newest first)
      def all
