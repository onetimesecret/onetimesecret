        ...
        module InstanceMethods
          ...
          def decrypted_value(opts = {})
            ...
            begin
              ...
              if allow_nil
                OT.li "[decrypted_value] m:#{metadata_identifier} s:#{key} Trying nil global secret"
          ...
          def try_fallback_secrets(encrypted_value, opts)
            ...
            rotated_secrets = OT.conf['experimental'].fetch('rotated_secrets', [])
            OT.ld "[try_fallback_secrets] m:#{metadata_identifier} s:#{key} Trying rotated secrets (#{rotated_secrets.length})"
            rotated_secrets.each_with_index do |fallback_secret, index|
              ...
              OT.ld "[try_fallback_secrets] m:#{metadata_identifier} s:#{key} Failed (index #{index})"
              next
            end
