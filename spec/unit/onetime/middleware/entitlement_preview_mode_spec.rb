# spec/unit/onetime/middleware/entitlement_preview_mode_spec.rb
#
# frozen_string_literal: true

require 'spec_helper'

# Unit tests for EntitlementPreviewMode Middleware
#
# Tests the middleware that copies session[:entitlement_preview_planid]
# to Thread.current[:entitlement_preview_planid] for the duration of each request.
#
RSpec.describe 'Rack::EntitlementPreviewMode' do
  ...
  let(:middleware_class) do
    Class.new do
      ...
      def call(env)
        ...
        if session && session[:entitlement_preview_planid]
  ...
  let(:app) do
    lambda { |env|
      ...
      [200, { 'Content-Type' => 'text/plain' }, [Thread.current[:entitlement_preview_planid].to_s]]
    }
  end
  ...
  describe '#call' do
    context 'when session has entitlement_preview_planid' do
      it 'copies value to Thread.current before request' do
        env = {
          'rack.session' => { entitlement_preview_planid: 'identity_v1' },
        }
      ...
      it 'sets Thread.current for the app to read' do
        ...
        app_that_checks = lambda { |_env|
          called_with = Thread.current[:entitlement_preview_planid]
          [200, {}, ['ok']]
        }
        middleware = middleware_class.new(app_that_checks)

        env = {
          'rack.session' => { entitlement_preview_planid: 'multi_team_v1' },
        }
      ...
      it 'handles different plan IDs' do
        ...
        test_plans.each do |planid|
          env = {
            'rack.session' => { entitlement_preview_planid: planid },
          }
    ...
    context 'when session has no entitlement_preview_planid' do
    ...
    describe 'cleanup behavior' do
      it 'clears Thread.current after successful request' do
        ...
        expect(Thread.current[:entitlement_preview_planid]).to be_nil
      end
      ...
      it 'clears Thread.current even when app raises exception' do
        failing_app = lambda { |_env|
          ...
          expect(Thread.current[:entitlement_preview_planid]).to eq('identity_v1')
    ...
    describe 'thread isolation' do
      it 'does not leak override to other threads' do
        ...
        thread = Thread.new do
          other_thread_value = Thread.current[:entitlement_preview_planid]
        end
        thread.join
    ...
    describe 'session value changes' do
      it 'updates Thread.current when session value changes' do
        session = { entitlement_preview_planid: 'free' }
      ...
      it 'handles session value cleared between requests' do
        ...
        # Clear session value
        session.delete(:entitlement_preview_planid)
        env2 = { 'rack.session' => session }
        _status, _headers, body2 = middleware.call(env2)
    ...
    describe 'edge cases' do
      it 'handles nil session value' do
        env = {
          'rack.session' => { entitlement_preview_planid: nil },
        }
      ...
      it 'handles empty string session value' do
        env = {
          'rack.session' => { entitlement_preview_planid: '' },
        }
