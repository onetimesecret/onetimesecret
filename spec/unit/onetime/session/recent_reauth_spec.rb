...
RSpec.describe Onetime::RecentReauth do
  ...
  describe '.satisfied?' do
    ...
    it 'refuses when the stored :at is missing or non-integer' do
      broken = { described_class::KEY => { account_id: 42, at: nil, surface: { kind: :canonical }, methods: [] } }
  ...
  describe 'NON_LOCAL_PRIMARIES constant' do
    it 'names the primaries callers must gate on before recording' do
      expect(described_class::NON_LOCAL_PRIMARIES).to contain_exactly('email_auth', 'omniauth')
    end
  end
end
