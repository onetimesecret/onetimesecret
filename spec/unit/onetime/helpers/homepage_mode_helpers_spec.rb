...
RSpec.describe Onetime::Helpers::HomepageModeHelpers do
  ...
  describe '.extract_x_forwarded_for' do
    ...
    it 'extracts single IP from X-Forwarded-For header' do
      ...
      expect(Onetime::ClientIpHelpers.extract_x_forwarded_for(env)).to eq(['203.0.113.0'])
    end

    it 'extracts multiple IPs from X-Forwarded-For header' do
      env['HTTP_X_FORWARDED_FOR'] = '203.0.113.0, 198.51.100.0, 192.0.2.0'
      expect(Onetime::ClientIpHelpers.extract_x_forwarded_for(env)).to eq(['203.0.113.0', '198.51.100.0', '192.0.2.0'])
    end

    it 'strips whitespace from IPs' do
      env['HTTP_X_FORWARDED_FOR'] = '  203.0.113.0  ,  198.51.100.0  '
      expect(Onetime::ClientIpHelpers.extract_x_forwarded_for(env)).to eq(['203.0.113.0', '198.51.100.0'])
    end

    it 'returns nil when header is not present' do
      expect(Onetime::ClientIpHelpers.extract_x_forwarded_for(env)).to be_nil
    end
  ...
  describe '.extract_rfc7239_forwarded' do
    ...
    it 'extracts single IP from RFC 7239 Forwarded header' do
      env['HTTP_FORWARDED'] = 'for=203.0.113.0'
      expect(Onetime::ClientIpHelpers.extract_rfc7239_forwarded(env)).to eq(['203.0.113.0'])
    end

    it 'extracts multiple IPs from Forwarded header' do
      env['HTTP_FORWARDED'] = 'for=203.0.113.0, for=198.51.100.0'
      expect(Onetime::ClientIpHelpers.extract_rfc7239_forwarded(env)).to eq(['203.0.113.0', '198.51.100.0'])
    end
    ...
    it 'extracts IPv6 addresses with brackets' do
      env['HTTP_FORWARDED'] = 'for="[2001:db8::1]"'
      expect(Onetime::ClientIpHelpers.extract_rfc7239_forwarded(env)).to eq(['2001:db8::1'])
    end

    it 'extracts IPs from complex Forwarded header with multiple parameters' do
      env['HTTP_FORWARDED'] = 'for=203.0.113.0;by=198.51.100.0, for=192.0.2.0'
      expect(Onetime::ClientIpHelpers.extract_rfc7239_forwarded(env)).to eq(['203.0.113.0', '192.0.2.0'])
    end
    ...
    it 'returns nil when header is not present' do
      expect(Onetime::ClientIpHelpers.extract_rfc7239_forwarded(env)).to be_nil
    end
