...
RSpec.describe Onetime::Jobs::Workers::NotificationWorker do
  ...
  before do
    ...
    allow(operation_instance).to receive(:call).and_return({ in_app: :success })
  end
  ...
  describe 'operation result handling' do
    let(:message) do
      JSON.generate(
        ...
        template: 'secret_viewed',
        channels: %w[in_app email webhook],
        data: {}
      )
    end
